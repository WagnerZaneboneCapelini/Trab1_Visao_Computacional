...
ENV VIRTUAL_ENV /opt/Trab1_Visao_Computacional-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/Trab1_Visao_Computacional/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/Trab1_Visao_Computacional/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/trab1_visao_computacional/ && touch src/trab1_visao_computacional/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/Trab1_Visao_Computacional/
